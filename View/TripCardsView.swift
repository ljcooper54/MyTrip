        ...
        if trips.isEmpty {
            ...
            GeometryReader { geo in
                VStack(spacing: 8) {
                    ...
                    // The built-in page dots above are the main affordance,
                    // but we also show a tiny hint text when there are multiple cards.
                    if trips.count > 1 {
...
struct TripCardView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            // Photo area: shows selected image or standard photo icon with picker
    ...
    private func handlePhotoSelection(_ item: PhotosPickerItem?) async {
        ...
        // Prefer storing the Photos local identifier (no image data copy)
        ...
        // Fallback: load image data just for display (not stored persistently)
        do {
